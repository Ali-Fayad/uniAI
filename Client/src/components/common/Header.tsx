...
const Header: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-[var(--color-surface)] shadow-sm border-b border-[var(--color-border)]">
          ...
          <div className="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0 gap-2">
            {isAuthenticated && (
              <button
                ...
                className="relative rounded-full bg-transparent px-4 py-2 text-sm font-medium text-[var(--color-textSecondary)] hover:text-[var(--color-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-focusRing)] focus:ring-offset-2 transition-colors"
              >
                Logout
              </button>
            )}
