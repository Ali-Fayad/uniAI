...
The same retrieval path now also supports a bounded `GRADUATE_OVERVIEW` intent that combines the existing program and tuition flows without moving SQL ownership into the service layer.
...
It also preserves `GRADUATE_OVERVIEW` across broad university questions, `both` replies, and same-question university replacement without introducing a generic dialogue manager.
It now also routes bounded casual and clearly unrelated messages through the normal main AI response without graduate retrieval; ambiguous and unsupported graduate requests remain on their existing protected clarification paths.
