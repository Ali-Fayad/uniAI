package com.uniai.shared.infrastructure.web;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<?> handleCVAccessDeniedException(Exception ex) {
        logger.warn("CVAccessDeniedException: {}", ex.getMessage(), ex);
        return ResponseEntity.status(403).body(ex.getMessage());
    }
    ...
    public ResponseEntity<?> handleJsonProcessingException(Exception ex) {
        logger.warn("JsonProcessingException: {}", ex.getMessage(), ex);
        return ResponseEntity.badRequest().body(ex.getMessage());
    }
