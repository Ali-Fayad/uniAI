...
public final class GraduateKnowledgeContextPolicyClassifier {
    ...
    public static GraduateKnowledgeContextPolicy classify(String message, List<ResolvedUniversity> explicitUniversities) {
        ...
        boolean comparison = containsAny(normalized,
        ...
        boolean referential = containsAny(normalized,
        ...
        boolean explicitScope = explicitUniversities != null && !explicitUniversities.isEmpty()
                || containsAny(normalized, " in beirut", " in byblos", " in tripoli", " in saida",
                " in sidon", " in nabatieh", " in zahle", " in jounieh", " in baalbek", " in lebanon");
    ...
    private static boolean containsAny(String value, String... signals) {
        for (String signal : signals) {
            if (value.contains(signal)) return true;
        }
        return false;
    }
}
